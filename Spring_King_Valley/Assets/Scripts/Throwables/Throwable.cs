    ...
    public class Throwable : MonoBehaviour
    {
        ...
        protected virtual void Awake()
        {
            ...
            if (throwableSpriteRenderer == null)
                throwableSpriteRenderer = GetComponentInChildren<SpriteRenderer>(true);
            if (shadowSpriteRenderer == null)
            {
                SpriteRenderer[] renderers = GetComponentsInChildren<SpriteRenderer>(true);
                if (renderers != null && renderers.Length > 1)
                    shadowSpriteRenderer = renderers[1];
            }
        }
        ...
        public virtual void Setup(int throwableIndex, Vector2 position, float damage, float knockback, float timeInAir, LayerMask targetLayer)
        {
            ...
            if (throwableSpriteRenderer != null)
            {
                ...
                Color c = throwableSpriteRenderer.color;
                if (c.a < 0.55f)
                {
                    c.a = 0.9f;
                    throwableSpriteRenderer.color = c;
                }
            }
            if (shadowSpriteRenderer != null)
