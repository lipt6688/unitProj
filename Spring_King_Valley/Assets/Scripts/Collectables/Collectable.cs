...
namespace Vampire
{
    public abstract class Collectable : MonoBehaviour
    {
        ...
        protected virtual bool AutoCollectInRange => false;
        protected virtual float AutoCollectRange => 0f;
        protected virtual bool IgnoreInventoryCapacity => false;
        ...
        public virtual void Setup(bool spawnAnimation = true, bool collectableDuringSpawn = true) {
            ...
            if (magnetic && entityManager != null)
        ...
        {
            if (!AutoCollectInRange || beingCollected || entityManager == null || playerCharacter == null || col == null || !col.enabled)
                return;

            float r = AutoCollectRange;
            if (r <= 0f)
                return;

            Vector3 center = GetPlayerCollectTarget();
            if ((center - transform.position).sqrMagnitude <= r * r)
                Collect();
        }
        ...
        public virtual void Collect(CollectionMode collectionMode = CollectionMode.FromGround)
        {
            ...
            if (beingCollected || entityManager == null || playerCharacter == null)
                return;

            // Don't collect if storable in inventory and inventory is full
            bool storeInInventory = !IgnoreInventoryCapacity && CollectableType != null && CollectableType.inventoryStackSize > 0;
            InventorySlot inventorySlot = null;
            bool hasInventorySlot = false;
            if (storeInInventory && entityManager.Inventory != null)
                hasInventorySlot = entityManager.Inventory.TryGetInventorySlot(this, out inventorySlot);
            ...
            if (storeInInventory && hasInventorySlot && inventorySlot != null)
        ...
        protected virtual IEnumerator FlyToPlayer(CollectionMode collectionMode = CollectionMode.FromGround)
        {
            ...
            Vector3 collectTarget = GetPlayerCollectTarget();
            float distance = Vector2.Distance(transform.position, collectTarget);
            ...
            while (t < 1)
            {
                ...
                if (lerpT >= 1) break;
                collectTarget = GetPlayerCollectTarget();
                transform.position = Vector3.LerpUnclamped(pickupPos, collectTarget, lerpT);
                yield return null;
            }
            transform.position = GetPlayerCollectTarget();
            yield return null;
            OnCollected();
        }

        private Vector3 GetPlayerCollectTarget()
        {
            if (playerCharacter == null)
                return transform.position;

            Transform center = playerCharacter.CenterTransform;
            if (center != null)
        ...
        {
            if (playerCharacter != null && playerCharacter.CollectableCollider != null && collider == playerCharacter.CollectableCollider)
            {
                Collect();
            }
        }
