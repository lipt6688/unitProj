...
namespace Vampire
{
    public class MeleeMonster : Monster
    {
        ...
        {
            if (col == null || col.collider == null || playerCharacter == null || monsterBlueprint == null)
                return;

            if (kingCityCalmActive)
                return;

            bool hitsPlayer = col.collider.CompareTag("Player") || col.collider.GetComponentInParent<PlayerHealth>() != null;
            if (!hitsPlayer)
                return;

            bool layerAllows = (monsterBlueprint.meleeLayer & (1 << col.collider.gameObject.layer)) != 0;
            bool fallbackAllows = monsterBlueprint.meleeLayer.value == 0;
            float attackInterval = monsterBlueprint.atkspeed > 0.01f ? 1.0f / monsterBlueprint.atkspeed : 1.0f;

            if (alive && (layerAllows || fallbackAllows) && timeSinceLastAttack >= attackInterval)
            {
